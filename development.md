...
- `components/session-win.tsx`：闭环结束时的 Session Win 卡片（纯前端）。
- `lib/session-win.ts`：从 diagnose/practice/coach/chat 结果构造 wins 与 next CTA；welcome-back。
- `lib/api-client.ts`：所有后端请求的统一入口（含 practice 的 `sessionSlot` / `sessionSize`）。
- `lib/types.ts`：前端使用的 TypeScript 数据结构。
- `lib/i18n.ts`：中英文文案（含 `sessionWin`）。
- `lib/llm-settings.ts`：模型选择和 BYOK localStorage。

读前端时建议选一条“结束路径”跟完，而不是只看首页：

```text
用户完成一次 Diagnose
  -> diagnostic-report 渲染报告
...
4. 跟读一次 Session Win：完成 diagnose → 看 end card → 清/改 localStorage 观察 welcome-back。
5. 跑全部测试。
6. 理解 Docker、Nginx、CORS、Vercel build-time env；理解 Oracle 日常源站 + Alibaba 同 SHA 待命。
7. 在 feature branch 上走一次 Preview，不直接改 main。
