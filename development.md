...
>
> 真实代码已落地在 `backend/`（FastAPI）与 `frontend/`（v0 + 集成 kit）。以下覆盖适用：
>
...
>    见 `backend/app/services/ai_client.py`。`.env` 改用 `DEEPSEEK_API_KEY` /
...
>    见 `frontend/V0_PROMPT.md` 与 `frontend/README.md`（第 4.1 节据此替换）。
> 3. **DynamoDB Decimal 修复：** boto3 不接受 Python `float`，写入前 float→Decimal、
>    读出后 Decimal→int/float。见 `backend/app/db/serialization.py`（第 13/14 节据此增强）。
...
> 6. **依赖与运行：用 uv 管理**（`backend/pyproject.toml` + `uv.lock`，Python 锁 3.11），
...
```txt
backend/app/services/diagnose_service.py
```
...
```txt
backend/app/api/routes/diagnose.py
```
...
Build this project as a frontend/backend separated app.

Architecture:
- frontend/ is a Next.js app deployed to Vercel.
- backend/ is a FastAPI app deployed to a Linux server.
