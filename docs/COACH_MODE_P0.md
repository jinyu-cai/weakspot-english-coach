...
| `generationMode` | `fast \| deep`，默认 `fast`；决定任务脚手架使用当前模型组合中的 Fast 或 Deep slot |
...
该接口使用 `rate_limited("coach")` 身份/配额策略，并最多读取 mastery 较低的五个技能作为个性化上下文。读取失败时仍可为新用户生成任务，但不伪造历史弱点。Chat 动态场景卡默认选择 Fast，用户可以显式改为 Deep；选择只影响新场景/任务的生成，不会把服务器 key 暴露给浏览器。
