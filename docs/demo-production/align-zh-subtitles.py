#!/usr/bin/env python3
"""Align the reviewed Chinese captions to the real Qwen sentence timings."""
...
REVIEW_SRT = PRODUCTION_DIR / "subtitles-zh-review.srt"
TIMELINE = PRODUCTION_DIR / "output" / "timeline-qwen.json"
OUTPUT_SRT = PRODUCTION_DIR / "output" / "subtitles-zh.srt"
...
def main() -> None:
    ...
    if len(captions) != len(segments):
        raise ValueError(
            f"Chinese captions ({len(captions)}) do not match Qwen segments ({len(segments)})."
        )
